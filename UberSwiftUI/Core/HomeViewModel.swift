...
//  UberSwiftUITutorial
//
//  Created by Aman Jain on 01/08/23.
//
...
class HomeViewModel: NSObject, ObservableObject {
    ...
    func deleteTrip() {
        ...
        Firestore.firestore().collection("trip").document(trip.id).delete { _ in
            self.trip = nil
        }
    }
}
    ...
    func addTripObserverForPassenger() {
        ...
        Firestore.firestore().collection("trips")
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                self.trip = trip
                print("DEBUG: Update trip stste is \(trip.state)")
        }
    }
...
extension HomeViewModel {
    ...
    func rejectTrip() {
        updateTripState(state: .rejectrd)
    }
